...
#include <algorithm>
// Define the necessary classes like Server, KubernetesCluster, Pod, Container, etc.
// Ensure exception handling is included where required.
...
int main() {
    ...
    try {
        ...
        std::cerr << "Exception capturée : Allocation Error: Serveur " << failNode.get_id() << " est inactif\n";
    }
    
    std::cout << "=== Test FileException ===\n";
    ...
    std::cout << "=== Test Lambda : serveurs inactifs ===\n";
    ...
    for (auto server : inactifs){
        std::cout << *server << "\n";
    }

    std::cout << "=== Déploiement sur un serveur inactif ===\n";
    ...
    try {
        ...
        std::cerr << "Exception capturée : Allocation Error: Server node3 n'est pas actif\n";
    }

    std::cout << "=== Pods triés par nombre de conteneurs ===\n";
    ...
    sort(pods.begin(), pods.end(), [](const std::unique_ptr<Pod>& a, const std::unique_ptr<Pod>& b) {
        return a->get_containers_size() > b->get_containers_size();
    });
    ...
    std::cout << "=== Tri des pods ===\n";
    for (auto &pod : pods){
        std::cout << *pod<<'\n';
    }

    std::cout << "=== Tous les conteneurs du cluster 1 ===\n";
    for (auto &pod : pods){
...
//     // Afficher ’létat après suppression du conteneur
...
//     // Afficher ’létat après suppression du pod
